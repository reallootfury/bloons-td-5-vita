...
void *find_exidx_soloader(uintptr_t pc, int *count) {
    if (count) {
        *count = 0;
    }

    if (!so_mod.exidx_base ||
        pc < so_mod.text_base || pc >= so_mod.text_base + so_mod.text_size) {
        return NULL;
    }

    if (count) {
        *count = (int)so_mod.exidx_count;
    }
    return (void *)so_mod.exidx_base;
}
...
void bionic_memory_barrier(void) {
    __sync_synchronize();
}

char * getenv_soloader(const char * var) {
    /* Bionic's libc and libcurl probe the standard proxy variables on every
     * worker startup.  Vita has no process environment for this loader, so a
     * missing value is the correct result and not a warning condition. */
    (void)var;
    return NULL;
}
