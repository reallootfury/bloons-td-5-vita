...
int errno_newlib_to_bionic(int error_number) {
	for (int i = 0; i < sizeof(errno_translation) / sizeof(errno_translation[0]); ++i) {
		if (error_number == errno_translation[i].errno_newlib)
			return errno_translation[i].errno_bionic;
	}

	l_error("Unexpected newlib errno %i, leaving it untranslated", error_number);
	return error_number;
}
