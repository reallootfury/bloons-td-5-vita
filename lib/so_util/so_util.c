...
int _so_load(so_module *mod, SceUID so_blockid, void *so_data, uintptr_t load_addr) {
    ...
    /*
     * Keep the loaded ARM exception-index range after the temporary ELF image
     * is released. Android's libunwind asks __gnu_Unwind_Find_exidx for this
     * table when a C++ exception crosses game frames.
     */
    for (int i = 0; i < mod->ehdr->e_phnum; i++) {
        if (mod->phdr[i].p_type == PT_ARM_EXIDX) {
            mod->exidx_base = mod->text_base + mod->phdr[i].p_vaddr;
            mod->exidx_count = mod->phdr[i].p_memsz / 8;
            break;
        }
    }
